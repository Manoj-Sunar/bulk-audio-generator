...
const QUICK_STEPS = [
  {
    ...
    description: "Select ElevenLabs or Google AI Studio",
  },
  {
    number: 2,
    title: "Enter API Key",
    description: "Paste your TTS provider API key",
  },
  {
    number: 3,
    title: "Paste Scripts",
    description: "Separate with blank lines",
  },
  {
    number: 4,
    title: "Generate & Download",
    description: "Get your ZIP file instantly",
  },
] as const;
...
export const DocumentationHero = () => {
  return (
    <section className="relative overflow-hidden py-20 lg:py-32">
      {/* ✅ Simplified background — 2 layers मात्र */}
      <div className="absolute inset-0 -z-10">
        <div className="absolute left-0 top-0 h-96 w-96 rounded-full bg-primary/15 blur-3xl" />
        <div className="absolute right-0 bottom-0 h-96 w-96 rounded-full bg-secondary/15 blur-3xl" />
      </div>
      ...
      <div className="mx-auto flex max-w-7xl flex-col items-center gap-12 px-6 lg:flex-row lg:gap-20">
        ...
        <div className="flex-1">
          ...
          <Paragraph
            size="lg"
            className="mt-6 max-w-2xl leading-8 text-on-surface-variant"
          >
            Transform your scripts into high-quality AI voices in bulk. Choose
            between ElevenLabs or Google AI Studio Text-to-Speech, configure
            your API key, generate hundreds of audio files, and download them
            instantly — all in one powerful platform.
          </Paragraph>
        ...
        <div className="flex-1">
          <div className="animate-float">
            <Card className="overflow-hidden rounded-[32px] border-primary/10 bg-white/80 shadow-2xl backdrop-blur-xl">
              <CardContent className="space-y-6 p-6 md:p-8">
                ...
                <div className="space-y-3">
                  {QUICK_STEPS.map((step) => (
                    ...
                    >
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-secondary text-white font-bold text-sm shadow-md transition-transform group-hover:scale-110">
                        {step.number}
                      </div>
                      <div>
...
// ✅ StatCard — CSS only, कुनै Framer Motion छैन
...
const StatCard = ({ icon: Icon, value, label }: StatCardProps) => (
  <div className="group rounded-2xl border border-outline-variant bg-white/40 p-5 text-center backdrop-blur-sm transition-all hover:-translate-y-1 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/10">
