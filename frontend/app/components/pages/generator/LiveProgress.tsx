...
interface LiveProgressProps {
  ...
  logs: ProgressLog[];
  onCancel?: () => void;
  estimatedTimePerFile?: number; // seconds
  currentFile?: string;
}
...
export const LiveProgress = ({
  ...
  const stats = useMemo(() => {
    ...
    return { successCount, errorCount, processingCount };
  }, [logs]);
  ...
  // ETA calculation with adaptive speed
  useEffect(() => {
    if (!running || completed === 0) return;
    ...
    const interval = setInterval(() => {
      ...
      // Adaptive ETA based on current speed and remaining files
      const estimatedSeconds = remaining / Math.max(speed, 0.1);
      const adjustedSeconds = Math.max(estimatedSeconds, remaining * estimatedTimePerFile);
      
      if (adjustedSeconds < 60) {
        setEta(`${Math.ceil(adjustedSeconds)}s`);
      } else {
        const mins = Math.floor(adjustedSeconds / 60);
        const secs = Math.ceil(adjustedSeconds % 60);
  ...
  useEffect(() => {
    if (completed === total && total > 0 && running === false) {
    ...
    <motion.div variants={fadeInUp} initial="hidden" animate="visible">
      <Card className="group overflow-hidden rounded-2xl border-slate-200/60 bg-white/80 backdrop-blur-xl shadow-lg shadow-slate-200/30 hover:shadow-xl hover:shadow-indigo-200/20 transition-all duration-500">
        ...
        <CardContent className="space-y-5 py-5 px-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <motion.div 
              className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all duration-300 ${
                running
                  ? "bg-gradient-to-r from-amber-50 to-orange-50 text-amber-700 border border-amber-200/80"
                  : stats.errorCount > 0
                  ? "bg-gradient-to-r from-red-50 to-rose-50 text-red-700 border border-red-200/80"
          ...
          {/* Main Progress with animated bar */}
          <div className="space-y-3">
            ...
            <div className="relative h-3 overflow-hidden rounded-full bg-slate-100/80">
              <motion.div
                className="relative h-full rounded-full bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600"
                initial={{ width: 0 }}
            ...
            <div className="grid grid-cols-4 gap-2 pt-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-slate-600">{stats.successCount}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="p-1 rounded bg-amber-100/50">
                  <Loader2 size={12} className="text-amber-600 animate-spin" />
                </div>
                <span className="text-xs font-medium text-slate-600">{stats.processingCount}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="p-1 rounded bg-red-100/50">
                  <XCircle size={12} className="text-red-600" />
                </div>
                <span className="text-xs font-medium text-slate-600">{stats.errorCount}</span>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-medium text-slate-600">
                  {processingSpeed ? `${processingSpeed.toFixed(1)}/s` : '0/s'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-slate-50/80 to-indigo-50/80 border border-slate-200/60 px-4 py-2.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-slate-500">Elapsed: {formatElapsed()}</span>
            </div>
          </div>
          ...
          {running && currentFile && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex items-center gap-2 rounded-lg bg-indigo-50/50 border border-indigo-100/50 px-3 py-2"
            >
              <Waves size={14} className="text-indigo-500 animate-pulse" />
              <span className="text-xs text-slate-600 truncate flex-1">
                Generating: <span className="font-medium text-indigo-700">{currentFile}</span>
              </span>
            </motion.div>
          )}

          {/* Logs with enhanced styling */}
              ...
              <AnimatePresence mode="popLayout">
                {logs.slice().reverse().map((log, index) => (
                  <motion.div
                    ...
                    animate="visible"
                    layout
                    className="flex items-start gap-2.5 text-xs py-1.5 px-2.5 rounded-lg hover:bg-white/50 transition-colors group"
                  >
                    ...
                    <span className="text-slate-400 whitespace-nowrap font-mono text-[10px] opacity-50 group-hover:opacity-100 transition-opacity">
                      {log.time}
                    </span>
                  </motion.div>
                ))}
            ...
            {running && (
              ...
              >
                <Button
                  ...
                  onClick={onCancel}
                  className="rounded-xl bg-gradient-to-r from-red-500 to-rose-500 text-white hover:from-red-600 hover:to-rose-600 shadow-lg shadow-red-500/25 hover:shadow-red-500/35 transition-all duration-300"
                >
                  Cancel Generation
                </Button>
              </motion.div>
            ...
            {showCelebration && (
              ...
              >
                <div className="text-center">
                  ...
                  <Heading as="h4" size="md" className="mt-2 text-slate-800">
                    All {total} files generated!
                  </Heading>
