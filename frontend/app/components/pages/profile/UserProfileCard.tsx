...
// ✅ Shared Generation type — ProfilePage बाट pass हुन्छ
...
interface UserProfileCardProps {
  user: UserType;
  generations: Generation[]; // ✅ Parent बाट data — duplicate fetch छैन
}

export const UserProfileCard = memo(({ user, generations }: UserProfileCardProps) => {
  // ✅ useMemo — सबै stats एकैचोटि compute
  const stats = useMemo(() => {
    ...
    const totalAudioFiles = generations.reduce(
  ...
  // ✅ Static stats array — render भित्र नयाँ नबनाउने
  const statItems = useMemo(
    () => [
      ...
      },
      {
        icon: Activity,
        label: 'Processing',
        value: stats.processingGenerations,
        color: 'amber',
      },
      {
        icon: FileAudio,
          ...
          {statItems.map((stat, idx) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                <div
                  className={`p-1.5 rounded-lg bg-${stat.color}-100/70 text-${stat.color}-600`}
                >
                ...
                <div>
                  <Paragraph
                    size="xs"
                    className="text-slate-500 font-medium leading-none"
                  >
                    {stat.label}
                  </Paragraph>
                  <Paragraph
                    size="sm"
                    weight="bold"
                    className="text-slate-800 leading-tight mt-0.5"
                  >
                    {stat.value}
                  </Paragraph>
                </div>
