...
export const ForgotPassword = () => {
  ...
  const handleOTPVerified = (otp: string) => {
    setOtp(otp);
    window.location.href = `/bulk-audio/reset-password?email=${encodeURIComponent(email)}&otp=${encodeURIComponent(otp)}`;
  };

  const currentStep = step === "request" ? 1 : 2;
            ...
            <div className="text-center">
              ...
              >
                <Span className="material-symbols-outlined text-4xl text-primary drop-shadow-md">
                  {step === "request" ? <LockKeyhole size={32} className="text-primary drop-shadow-md" />: <CircleCheckBig size={32} className="text-primary drop-shadow-md" />}
                </Span>
              </motion.div>
              <Heading
                as="h1"
