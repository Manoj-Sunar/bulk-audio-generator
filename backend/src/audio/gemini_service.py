...
def generate_audio_for_chunks(
    ...
    """
    Generate audio for each chunk using Gemini TTS.
    Returns (list of (title, wav_bytes), total_tokens_used).
    """
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            raise HTTPException(500, f"Failed to parse Gemini response for chunk {idx}: {str(e)}")
        ...
        try:
            ...
            logger.error(f"Failed to decode audio for chunk {idx}: {str(e)}")
            raise HTTPException(500, f"Failed to decode audio for chunk {idx}: {str(e)}")

        # Convert PCM to WAV (24kHz, mono, 16-bit)
        try:
            wav_bytes = pcm_to_wav(pcm_bytes)
        except Exception as e:
            logger.error(f"Failed to convert PCM to WAV for chunk {idx}: {str(e)}")
            raise HTTPException(500, f"Failed to convert PCM to WAV for chunk {idx}: {str(e)}")
        ...
        results.append((title, wav_bytes))
